 ...
 * @type : function
 * @access : public
 * @desc : checking certain priv from session, a_buttonpriv: ex)
 *       "sto_bttn_able_yn"
 */
function gf_GetPriv(scope, a_buttonpriv, SessionInfo) {
  var hasPriv = false;
  if (SessionInfo) {
    scope.SessionInfo = angular.copy(SessionInfo.init());
  }
  var currentUrl;
  if (scope.$root.$state) {
    currentUrl = scope.$root.$state.current.name;
  } else {
    currentUrl = document.location.href;
    currentUrl = currentUrl.substring(currentUrl.lastIndexOf('/') + 1,
        currentUrl.length);
  }
  var priv = scope.SessionInfo.getCurrentPriv();
  if (priv === null || priv.length === 0) {
    hasPriv = true; // arbitrarily
  } else {
    for (var i = 0; i < priv.length; i++) {
      if (priv[i].menu_url === currentUrl) {
        if ('Y' === priv[i][a_buttonpriv]) {
          hasPriv = true;
        } else {
          hasPriv = false;
        }
